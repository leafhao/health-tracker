...
METRIC_CATALOG: dict[str, dict[str, Any]] = {
    ...
    "body_mass_kg": _metric("body", "kg", "HealthKit BodyMass", "只取分析日前 30 天内最近一次测量，并同时返回 body_mass_at", "体重趋势背景", "超过 30 天没有测量时返回空值"),
    "body_fat_percent": _metric("body", "%", "HealthKit BodyFatPercentage", "只取分析日前 30 天内最近一次测量，统一为百分数并返回 body_fat_at", "身体成分趋势", "体脂秤结果属于消费级估算；超过 30 天没有测量时返回空值"),
    "body_mass_index": _metric("body", "kg/m²", "优先 HealthKit BodyMassIndex，否则 BodyMass 与 Height", "直接读取或按体重÷身高²计算；体重或 BMI 测量必须在近 30 天内，读取 body_mass_index_source 区分", "体重相对身高的筛查指标", "不能单独反映肌肉与脂肪分布"),
    "lean_body_mass_kg": _metric("body", "kg", "优先 HealthKit LeanBodyMass，否则 BodyMass 与 BodyFatPercentage", "直接读取或按体重×(1-体脂率)计算；身体成分测量必须在近 30 天内，读取 lean_body_mass_source 区分", "非脂肪身体质量趋势", "计算精度受体脂秤估算误差影响"),
