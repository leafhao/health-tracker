...
- VO₂ Max 和心率恢复使用目标日期结束前的最近一次有效测量；体重和体脂率仅使用分析日前 30 天内的测量，避免陈旧身体成分误导。
- BMI 优先读取近 30 天 HealthKit 测量；缺失时按近 30 天体重和长期身高计算。去脂体重优先读取近 30 天 HealthKit 测量；缺失时按近 30 天体重和体脂率计算。结果保留测量时间和来源标记。
