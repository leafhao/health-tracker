...
- 快捷指令：公开“立即增量同步”和“训练结束后同步”；所有请求先持久化并合并，训练动作还会安排约 10 分钟后的复查机会。
- 可靠性：设置页可查看/导出最近同步流水，并可开启超过 24 小时未同步提醒；小包先短时直传，失败后由系统后台 URLSession 接管。
...
建议家庭和公司分别建立一条自动化，并优先增加“连接指定 Wi-Fi”这一更明确的触发。App 会先保存请求，再利用本次自动化的执行时间完成短增量采集和后台上传交接；手机锁定、执行时间不足或网络异常时，请求继续保留。若已有同步任务运行，新请求会被合并并留待后续消费。设置页可查看最近触发、最近完成和等待原因；如果 Apple Watch 数据在触发后才进入 iPhone HealthKit，HealthKit Observer 或下一次自动化仍会补上。
