...
- 快捷指令：公开“立即增量同步”后台动作，可绑定到达家、到达公司、Wi-Fi、充电器等个人自动化；动作先限时直传一个合并包，失败自动转入后台，不打开 App、不回溯历史。
...
建议家庭和公司分别建立一条自动化。动作会在快捷指令结果中区分“已立即上传”“已转入后台队列”和“仅保存在手机等待重试”；若触发时已有同步任务运行，请求会被合并，并在当前任务结束后自动补查一次。重复触发不会重复入库；如果 Apple Watch 数据在触发后才进入 iPhone HealthKit，HealthKit Observer 或下一次自动化仍会补上。
