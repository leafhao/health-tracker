...
struct HealthPermissionsView: View {
        ...
        List {
            ...
            Section("Individual Item Access") {
                VStack(alignment: .leading, spacing: 8) {
                    Text("Some data types require you to individually select which items to share. Tap each button below to choose which items Health Beat can access.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
            ...
            if vm.permissionsRequested {
                Section {
                    VStack(alignment: .leading, spacing: 6) {
                        ...
                        Text("iOS only shows the Health permission dialog once per type. If tapping \"Re-request\" shows no dialog, your permissions are already set. To change access, open the Health app → Sharing → Apps → Health Beat.")
                            .font(.caption)
                            .foregroundStyle(.secondary)
                    }
                    .padding(.vertical, 2)
            ...
            Section {
                VStack(alignment: .leading, spacing: 6) {
                    Label("About permission status", systemImage: "info.circle")
                        .font(.caption.weight(.semibold))
                        .foregroundStyle(.secondary)
                    Text("iOS hides whether individual read permissions were granted or denied — this is by design. \"Processed\" means the HealthKit dialog was shown for that type. To change access, go to: Health app > Sharing > Apps > Health Beat.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
                .padding(.vertical, 2)
    ...
    private var statusSubtitle: String {
        ...
        if vm.deniedTypes.isEmpty {
            return "Health Beat has access to all requested data types."
        }
