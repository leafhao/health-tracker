...
actor CloudRelayTransport {
    ...
    /// Upload one stable, bounded pack while an explicit Shortcuts intent still
    /// owns foreground execution time. Location and charger automations should
    /// normally make their data visible within seconds, while a hard timeout
    /// leaves the encrypted outbox untouched for the background fallback.
    func uploadOnePendingImmediately(
        outbox: EncryptedHealthOutbox,
        pairing: HealthPairingMaterial,
        config: CloudStorageConfig,
        credentials: CloudStorageCredentials,
        ...
        do {
            ...
            // Replace a completed transient failure with an immediate attempt.
        }
        // A previous observer/shortcut may have handed the exact same stable
        // pack to background URLSession. Replace that discretionary task with
        // this bounded immediate attempt; a failed attempt is scheduled again
        // by the caller using the same transfer ID and object key.
        await backgroundTransfers.cancelAndDiscardCloudTransfers()
        ...
        await backgroundTransfers.cancelAndDiscard(transferID: transferID)
        return pack.batches.count
    }
    ...
    private func performImmediate(
        ...
        let session = session
        let rateLimitCount = consecutiveRateLimits + 1
        ...
        return try await withThrowingTaskGroup(of: Data.self) { group in
            group.addTask {
                ...
                if accepted.contains(http.statusCode) { return data }
                if http.statusCode == 429 {
                    throw CloudStorageError.rateLimited(
                        Self.rateLimitRetryDate(
                            response: http,
                            consecutiveRateLimits: rateLimitCount
                        )
                    )
                }
                throw CloudStorageError.http(
