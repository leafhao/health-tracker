...
- 快捷指令：App 公开“立即增量同步”后台动作，可由到达家、到达公司、Wi-Fi、充电器等个人自动化触发；不打开 App，也不执行历史回溯。
...
该动作查询所有 HealthKit anchored streams 的新增变化，在手机端加密并持久化，然后把一个 S3 密文包交给系统后台上传。它不打开 App、不等待 Receiver、不扫描历史，也不会主动弹出结果对话框。若触发时已有同步任务运行，本次请求会被合并，并在当前任务结束后自动补查一次；增量 anchor、稳定批次 ID 和 Receiver 幂等入库共同保证不会重复入库。
