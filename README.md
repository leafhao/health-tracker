...
- 身体成分：体重、体脂率、BMI、去脂体重及真实测量趋势；缺失的 BMI 和去脂体重可由 Receiver 透明计算并标注来源。
...
- 快捷指令：App 公开“立即增量同步”和“训练结束后同步”两个动作；后者会额外保存约 10 分钟后的训练复查任务。到达家、公司、Wi-Fi、充电器等触发产生的请求都会先落盘，再由统一协调器合并执行。
- 同步诊断：设置页保留最近 14 天的触发来源、阶段、计数和错误，可导出 JSON；Receiver 面板显示密文包从手机到面板物化的五段时间链。
...
动作会先持久化带时间戳的同步请求，再读取 HealthKit anchor 后的变化并端侧加密。小包会先尝试一次短时直传；未成功则把同一稳定合并包交给系统后台 `URLSession`。它不打开 App、不等待 Receiver 入库，也不扫描历史。若触发时手机锁定、执行时间不足或网络异常，请求仍会由解锁、后台刷新、HealthKit 变化唤醒或进入前台继续处理。多次触发会合并，增量 anchor、稳定批次 ID 和 Receiver 幂等入库共同避免重复。训练结束场景可改用“训练结束后同步”，让稍晚写入的路线和恢复指标得到一次延后复查。
